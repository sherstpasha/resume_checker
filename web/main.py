...
from utils import build_highlighted_resume, city_from_address
...
async def index(request: Request, q: str | None = None, status: str | None = None):
    sql = "SELECT id, candidate_status, created_at, (SELECT avg_score FROM analyses WHERE candidate_id=candidates.id ORDER BY id DESC LIMIT 1) AS avg_score FROM candidates"
    params = []
    where = []
    # Optional filter by id if q is numeric (hidden in UI)
    if q and q.isdigit():
        where.append("id = ?")
        params.append(int(q))
    if status:
    ...
    for r in last_rows:
        ...
        if not chosen:
            m = _re.search(r"�?������?�?��[���]\s+['\"]([^'\"]+)['\"]", s, flags=_re.IGNORECASE)
            if m:
...
async def candidate_detail(request: Request, cid: int):
    ...
    for it in analyses:
        ...
        # precompute safe data for templates
        grades_rows = []
        if isinstance(it.get("report"), dict):
            anal = it["report"].get("�?�?���>���") or {}
            if isinstance(anal, dict) and isinstance(anal.get("�?�Ő�?��� �'�?��+�?�?���?���"), dict):
                grades = anal.get("�?�Ő�?��� �'�?��+�?�?���?���") or {}
                for req_text, obj in grades.items():
                    if not isinstance(obj, dict):
                        continue
                    try:
                    ...
                    comment = ""
                    try:
                        comment = str(obj.get("��?�?�?��?�'���?���") or "")
                    except Exception:
                        comment = ""
                    grades_rows.append({"req": str(req_text), "score": score, "comment": comment})
            qs = None
            try:
                qs = it["report"].get("�'�?���?�?�?�<")
            except Exception:
                qs = None
            it["questions"] = qs if isinstance(qs, list) else []
        it["grades_rows"] = grades_rows
