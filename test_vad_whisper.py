...
def main():
    parser = argparse.ArgumentParser(description="Standalone mic → VAD → Whisper tester (CLI)")
    parser.add_argument("--model", default=os.getenv("WHISPER_MODEL", "medium"), help="whisper model (tiny/base/small/medium/large)")
    parser.add_argument("--device", default=os.getenv("STT_DEVICE", "auto"), help="auto|cuda|cpu")
    parser.add_argument("--samplerate", type=int, default=int(os.getenv("SAMPLERATE", "16000")))
    parser.add_argument("--chunk_sec", type=float, default=float(os.getenv("CHUNK_SEC", "1")))
    parser.add_argument("--pause_chunks", type=int, default=int(os.getenv("PAUSE_CHUNKS", "2")))
    parser.add_argument("--rms_gate", type=float, default=float(os.getenv("RMS_GATE", "0.01")), help="RMS threshold for fallback VAD")
    parser.add_argument("--device_index", type=int, default=None, help="sounddevice input device index")
    parser.add_argument("--list_devices", action="store_true", help="print sound devices and exit")
    ...
    print(f"🎚️ SR={samplerate}Hz, окно={chunk_sec:.1f}s, пауза={pause_chunks} окна, RMS_GATE={rms_gate}")
    ...
    while running:
        ...
        print(f"ℹ️ VAD 1s: rms={rms:.4f} has_speech={has_speech} (silero={has_speech_silero}, rms_gate={has_speech_rms})")
